function [outputs] = CHO_v22_nr(signal, srate, param)
...
for ifreq = 1:length(freqs)
    freq = freqs(ifreq);
    % applies a bandpass filter to the signal for the current frequency range
    ...
    % Computes the envelope of the filtered signal using the Hilbert transform (hilbert) and then takes the logarithm (base 10) of the absolute value of the envelope. aka amplitude in dB
    env_data(:,ifreq) = log10(abs(hilbert(filtered_data(:,ifreq))));
    med_env(ifreq) = median(env_data(:,ifreq)); % calculates the median of the envelope
end

%% select lowest one over f fit
clear med_psd
%  divides the total time range (timex) into four equal parts
q4_points = round(linspace(1,timex(end),5),0);
q4_start = q4_points(1:end-1);
q4_end = q4_points(2:end);
X = freqs';
for iq = 1:length(q4_start)
    med_psd(:,iq) = median(env_data(q4_start(iq):q4_end(iq),:),1);   % contains 1 median value for each time bin and each freq of the envelope
   
    Y = med_psd(:,iq);
    
    % fit the 1/f
    [oof_outputs] = oof_fitting_v2(Y,X);
    ap_fits(:,iq) = oof_outputs.init_ap_fit; % store the fitting result
end

% m_ap_fits = median(ap_fits);
[minval minidx] = min(ap_fits(1,:)); % selects the best aperiodic fit by looking at the first row of ap_fits (which corresponds to the lowest frequency)


%% remove pink noise
ap_fit = ap_fits(:,minidx); % selected aperiodic fit
ap_mat = repmat(ap_fit,1,length(timex)); % create a matrix for subtraction
% The aperiodic fit (ap_mat) is subtracted from the original data
% (env_data) to remove pink noise, = denoised signal:
denoised_env_data = env_data - ap_mat'; 
% denoised_env_data = denoised_env_data';
std_residue = std(denoised_env_data(:)); % std of the denoised matrix
...
%% masking osc data
osc_threshold = 2*std_residue; % this threshold will be used to identify significant oscillatory components in the data
...
for ifp = 1:length(freqs)
    sig_idxs = find(denoised_env_data(:,ifp)>osc_threshold); %identifies time points where the denoised envelope data at the current frequency exceeds the osc_threshold.
    oevent_map(sig_idxs,ifp) = 1; % Sets the entries corresponding to significant oscillatory events to 1. This creates a binary mask that marks the locations of significant oscillations for the current frequency.
    osc_data(sig_idxs,ifp) = filtered_data(sig_idxs,ifp) .* oevent_map(sig_idxs,ifp);
    masked_env_data(sig_idxs,ifp) = denoised_env_data(sig_idxs,ifp) .* oevent_map(sig_idxs,ifp); % significant oscillations (identified by oevent_map) are kept, while all other values are set to zero
end
...
%%
freq_pnts = find(sum(masked_env_data,1)>0); % somma le colonne e individua quali sono > 0
...
for ibox = 1:n_boxes
    ...
    if length(pos_ac_LOCS)>1 && length(neg_ac_LOCS)>1 % cycle is bigger than 2
        ...
        pos_herz = srate/pos_ac_LOCS(1);
        neg_herz = srate/neg_ac_LOCS(1);
        ...
        if pos_herz >= freqs(1) && pos_herz <= freqs(end)
            if bounding_boxes(ibox).minF <= pos_herz && bounding_boxes(ibox).maxF >= pos_herz && ( abs(pos_invterval_simliarity_btw_pos_pks - 100) < 30 || abs(neg_invterval_simliarity_btw_pos_pks - 100) < 30 )
                ...
                % I AM ADDING THIS TO AVOID AN ERROR 
                if bounding_boxes(ibox).maxF > freqs(end)
                    bounding_boxes(ibox).maxF = bounding_boxes(ibox).center_fp;
                end
